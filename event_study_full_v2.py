"""
Event Study 重建版(v2)：用 build_event_dataset.py / build_price_dataset.py 產出的
完整、去重、有分頁保證的資料集重跑，取代先前被1000筆上限截斷的 event_study_full.py。

方法不變：市場模型 alpha+beta_mkt*R_mkt+beta_ind*R_ind，估計窗[-250,-30]交易日，
...
for ev in events:
    ...
    rows.append({
        ...
        "analyst_count": ev["analyst_count"], "target_change_pct": ev.get("target_change_pct"),
...
with open("factset_data/full_issues_v2.json", "w", encoding="utf-8") as f:
    json.dump(issues, f, ensure_ascii=False, indent=2)
df.to_csv("factset_data/full_event_study_results_v2.csv", index=False)
...
print("\n=== 全樣本彙總（依方向，date-cluster標準誤，完整2023-2026資料）===")
